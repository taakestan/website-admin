...
export default class BaseModel extends StaticModel {
    ...
    args.forEach(value => {
      switch(true) {
        ...
        case (value instanceof BaseModel):
          ...
          break;
        default:
          throw new Error('Arguments to custom() must be strings or instances of BaseModel.')
      }
  ...
  for(...args) {
    ...
    args.forEach(object => {
      if (object instanceof BaseModel === false) {
        throw new Error('The object referenced on for() method is not a valid BaseModel.')
      }
